...
class AudioEngine:
    ...
    def __init__(self, model_size="base", language="auto", device="cpu"):
    ...
    def _transcribe_optimized(self, audio_data: np.ndarray, boost_phrases: List[str], fast_mode: bool = False) -> Optional[str]:
        ...
        try:
            ...
            whisper_language = None if self.language in (None, "", "auto") else self.language
            if BACKEND == "faster-whisper":
                ...
                _stt_log( f"Language detected: {self.last_detected_language}, "
                               f"probability={self.last_language_probability}")

                print(f"[AudioEngine] Language: {self.last_detected_language}, "
                         f"probability={self.last_language_probability}")
    ...
    def _filter_by_word_count(
        ...
        """
        Filter transcription length.
        English: count words.
        Chinese: count Chinese characters.
        """
        if not text:
            return text
        ...
        if self.last_detected_language == "zh":
            chinese_chars = re.findall(r'[\u4e00-\u9fff]', cleaned)

            if len(chinese_chars) > max_zh_chars:
                print(
                    f"[AudioEngine] Chinese text exceeds limit "
                    f"({len(chinese_chars)} chars): '{text}'"
                )
                _stt_log(
                    f"Rejected Chinese transcription: "
                    f"{len(chinese_chars)} chars > {max_zh_chars}"
                )
                return None

            return text

        # English / other space-separated languages
        words = cleaned.split()
        word_count = len(words)

        if word_count > max_words:
            print(
                f"[AudioEngine] Text exceeds word limit "
                f"({word_count} words): '{text}'"
            )
            _stt_log(
                f"Rejected transcription: "
